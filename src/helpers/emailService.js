...
const sendOrderOTPEmail = async (email, otp) => {
    console.log(`📝 Order OTP for ${email}: ${otp}`);
    ...
    if (isProduction && process.env.SENDGRID_API_KEY) {
        ...
        try {
            const mailOptions = {
                from: process.env.SENDGRID_FROM_EMAIL || 'noreply@cafetrack.com',
                to: email,
                subject: 'Order Verification OTP - CafeTrack',
                html: `
                    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                        <h2 style="color: #333; text-align: center;">Order Verification</h2>
                        <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
                            <p style="font-size: 16px; color: #333; margin-bottom: 20px;">
                                Your order verification code is:
                            </p>
                            <div style="text-align: center; margin: 30px 0;">
                                <span style="font-size: 32px; font-weight: bold; color: #007bff; letter-spacing: 5px;">
                                    ${otp}
            ...
            const info = await transporter.sendMail(mailOptions);
            console.log('✅ Order OTP email sent via SendGrid:', info.messageId);
            return { success: true, messageId: info.messageId };
        } catch (error) {
            console.error('❌ SendGrid email failed:', error.message);
            return { success: false, error: error.message };
        ...
        // Production without SendGrid - just log
        console.log('✅ Order OTP generated (email skipped - no SendGrid)');
        return { success: true, messageId: 'production-skip' };
    } else {
        // Local development - Gmail
        const transporter = nodemailer.createTransport({
        ...
        try {
            ...
            const info = await transporter.sendMail(mailOptions);
            console.log('✅ Order OTP email sent via Gmail:', info.messageId);
            return { success: true, messageId: info.messageId };
        } catch (error) {
            console.error('❌ Gmail email failed:', error.message);
            console.log(`📝 Order OTP for ${email}: ${otp} (Gmail failed)`);
            return { success: false, error: error.message };
        }
    }
};

export { generateOTP, sendOTPEmail, sendOrderOTPEmail };